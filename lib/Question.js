...
class Question {
  ...
  static create(text,desc,winners,...choices){
    ...
    require('./ensureWorkDirSync')(cfg.workdir);
    var file = `${cfg.workdir}/${inst.generateId()}.json`;
    var json = inst.json;    
  ...
  get idx(){
    var idx = Question.loadedIDs[this.id];
   ...
   */
  generateId(){  
    return ObjectHash(this.data,{algorithm:cfg.hash_algorithm});
  }
  /**
